...
($item->line_total > 0
    ...
    : ($item->total_amount > 0
        ...
        : ($item->quantity * $item->unit_price)))
            ...
            $isIntra = $invoice->place_of_supply === 'intra_state';
            @endphp
            ...
            <div class="totals-wrapper">
                <div class="totals-section">
                    <table class="totals-table">
                        ...
                        @else
                        @if($hasBreakdown)
                        {{-- Detailed breakdown (new invoices) --}}
                        @if($isIGST)
                        <tr>
                            <td>IGST @ {{ $effectiveRate }}%</td>
                            <td>Rs. {{ \App\Helpers\NumberToWords::indianFormat($igst) }}</td>
                        </tr>
                        @else
                        @if($cgst > 0)
                        <tr>
                            <td>CGST @ {{ round($effectiveRate / 2, 2) }}%</td>
                            <td>Rs. {{ \App\Helpers\NumberToWords::indianFormat($cgst) }}</td>
                        </tr>
                        @endif
                        @if($sgst > 0)
                        <tr>
                            <td>SGST @ {{ round($effectiveRate / 2, 2) }}%</td>
                            <td>Rs. {{ \App\Helpers\NumberToWords::indianFormat($sgst) }}</td>
                        </tr>
                        @endif
                        @endif
                        @elseif($totalGst > 0)
                        ...
                        @if($isIntra)
                        <tr>
                            <td>CGST + SGST ({{ $effectiveRate }}%)</td>
                            <td>Rs. {{ \App\Helpers\NumberToWords::indianFormat($totalGst) }}</td>
                        </tr>
                        @else
                        <tr>
                            <td>IGST @ {{ $effectiveRate }}%</td>
